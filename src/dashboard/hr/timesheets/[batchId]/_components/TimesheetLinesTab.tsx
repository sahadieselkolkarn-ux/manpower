                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        <FormField control={form.control} name="assignmentId" render={({ field }) => (
                         ...
                         <FormField control={form.control} name="workDate" render={({ field }) => (
                            <FormItem><FormLabel>Work Date</FormLabel><FormControl><Input placeholder={DATE_FORMAT} {...field} value={formatDate(field.value) || ''} disabled={isLocked} /></FormControl><FormMessage /></FormItem>
                        )}/>
                            ...
                            {isLoading ? (
                                ...
                                sortedLines.map(line => {
                                    ...
                                    <TableRow key={line.id} className={line.anomalies && line.anomalies.length > 0 ? 'bg-red-50 dark:bg-red-900/20' : ''}>
                                        <TableCell>{formatDate(line.workDate)}</TableCell>
